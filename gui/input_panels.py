...
class InputPanel(ttk.Frame):
    ...
    def update(self):
        self.init_spin.config(text=str(self.controller.init_spin))
        self.hangle.config(text=str(self.controller.hangle))
        self.altitude.config(text=str(self.controller.launch_altit))
        self.sim_len.config(text=str(self.controller.length))
        if self.controller.length != 0:
            self.sim_freq.config(text=str(self.controller.timesteps / self.controller.length))
        else:
            self.sim_freq.config(text='0')